...
export default function WaitingC() {
    ...
    let searchFilter = async (input) => {
        ...
        let response = await axios.post('http://localhost:8000/transaction/FWarehouse', { warehouse_city: input, order_status_id:2 })
        setDataTR(response.data.data)
    }


    let getAllTr = async () => {
        let response = await axios.post('http://localhost:8000/transaction/getAllTransaction', user.warehouse ? { warehouse: user.warehouse, order_status_id:1 } : {order_status_id:2})
        setDataTR(response.data.data)
